...
If UCase(Module1.GetCaption(firs%)) Like UCase(childhand) Then GoTo bone
firs% = GetWindow(parentw, GW_CHILD)

While firs%
firss% = GetWindow(parentw, 5)
If UCase(Module1.GetCaption(firss%)) Like UCase(childhand) & "*" Then GoTo bone
...
Public Sub SendMail2(person As String, Subject As String, Message As String)
...
Public Sub AddRoomToListboxWithAt(TheList As ListBox, AddUser As Boolean)
    On Error Resume Next
    Dim cProcess As Long, itmHold As Long, ScreenName As String
    Dim psnHold As Long, rBytes As Long, Index As Long, Room As Long
    Dim rList As Long, sThread As Long, mThread As Long
    ...
    If mThread& Then
        For Index& = 0 To SendMessage(rList, LB_GETCOUNT, 0, 0) - 1
            ...
            ScreenName$ = Left$(ScreenName$, InStr(ScreenName$, vbNullChar) - 1)
            If ScreenName$ <> GetUser$ Or AddUser = True Then
                TheList.AddItem ScreenName$ & "@aol.com"
            End If
        Next Index&
        Call CloseHandle(mThread)
    End If
...
Public Sub AddRoomToTextbox(TheList As TextBox, AddUser As Boolean)
    On Error Resume Next
    Dim cProcess As Long, itmHold As Long, ScreenName As String
    Dim psnHold As Long, rBytes As Long, Index As Long, Room As Long
    Dim rList As Long, sThread As Long, mThread As Long
    ...
    If mThread& Then
        For Index& = 0 To SendMessage(rList, LB_GETCOUNT, 0, 0) - 1
            ...
            ScreenName$ = Left$(ScreenName$, InStr(ScreenName$, vbNullChar) - 1)
            If ScreenName$ <> GetUser$ Or AddUser = True Then
                TheList.Text = TheList.Text & ScreenName$ & ", "
            End If
        Next Index&
        Call CloseHandle(mThread)
    End If
...
Public Sub AddRoomToTextboxWithAt(TheList As TextBox, AddUser As Boolean)
    On Error Resume Next
    Dim cProcess As Long, itmHold As Long, ScreenName As String
    Dim psnHold As Long, rBytes As Long, Index As Long, Room As Long
    Dim rList As Long, sThread As Long, mThread As Long
    ...
    If mThread& Then
        For Index& = 0 To SendMessage(rList, LB_GETCOUNT, 0, 0) - 1
            ...
            ScreenName$ = Left$(ScreenName$, InStr(ScreenName$, vbNullChar) - 1)
            If ScreenName$ <> GetUser$ Or AddUser = True Then
                TheList.Text = TheList.Text & ScreenName$ & "@aol.com" & ", "
            End If
        Next Index&
        Call CloseHandle(mThread)
    End If
...
Public Sub IMIgnore(person As String)
    Call InstantMessage("$IM_OFF, " & person$, "Fatal Fury")
End Sub

Public Sub IMUnIgnore(person As String)
    Call InstantMessage("$IM_ON, " & person$, "Fatal Fury")
End Sub
...
Public Sub SaveListBox(Directory As String, TheList As ListBox)
    ...
    On Error Resume Next
    Open "C:\list.PWS" For Output As #1
...
Public Sub FormExitDown(TheForm As Form)
    ...
    Loop Until TheForm.Top > Screen.Height + TheForm.Height
End Sub
...
Public Sub FormExitUp(TheForm As Form)
    Do
        ...
        DoEvents
        TheForm.Top = Trim(Str(Int(TheForm.Top) - 300))
    Loop Until TheForm.Top < 0 - TheForm.Height
End Sub
...
Call AOLSetText(subbas.AOLWindow(), newcaption)
End Sub
...
AOL% = FindWindow("AOL Frame25", vbNullString)
AOModal% = subbas.FindChildByClass(AOL%, "_AOL_Modal")
AOGauge% = subbas.FindChildByClass(AOModal%, "_AOL_Gauge")
...
'Exit Sub
'Begin:
'e = module1.FindChildByClass(IM%, "RICHCNTL")'
...
Public Function FindMsg() As Boolean
    Dim AOL As Long, dMsg As Long
    AOL& = FindWindow("AOL Frame25", vbNullString)
    dMsg& = FindWindowEx(AOL&, 0&, "#32770", vbNullString)
    If dMsg <> 0 Then
        FindMsg = True
    Else
        FindMsg = False
